<nav x-data="{ open: false, darkMode: localStorage.getItem('theme') === 'dark' }" x-init="$watch('darkMode', value => { document.documentElement.classList.toggle('dark', value); localStorage.setItem('theme', value ? 'dark' : 'light'); }); if (darkMode) document.documentElement.classList.add('dark');" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <!-- Settings Dropdown y Dark Mode Toggle (Vista Normal) -->
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <!-- Botón para alternar Dark Mode -->
                <button @click="darkMode = !darkMode" type="button" class="mr-4 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none" aria-label="Toggle dark mode">
                    <!-- Si está en modo oscuro, mostramos el icono de sol (para pasar a modo claro) -->
                    <template x-if="darkMode">
                        <i class="fas fa-sun"></i>
                    </template>
                    <!-- Si está en modo claro, mostramos el icono de luna (para pasar a modo oscuro) -->
                    <template x-if="!darkMode">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <!-- Responsive Settings Options y Dark Mode Toggle -->
        <div class="pt-4 pb-1 border-t border-gray-200 dark:border-gray-600">
            <div class="px-4 flex items-center justify-between">
                ...
                <!-- Botón responsive para alternar Dark Mode -->
                <button @click="darkMode = !darkMode" type="button" class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none" aria-label="Toggle dark mode">
                    <template x-if="darkMode">
                        <i class="fas fa-sun"></i>
                    </template>
                    <template x-if="!darkMode">
